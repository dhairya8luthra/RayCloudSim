...
class ZAM_env(Env_Trust):
    ...
    def _record_frame_info(self):
        while True:
            ...
            if len(self.config['VisFrame']['TargetNodeList']) > 0:
                self.frame_info[self.now]['target'] = {
                    item: [self.scenario.get_node(item).active_task_ids[:], self.scenario.get_node(item).task_buffer.task_ids[:]]
                    for item in self.config['VisFrame']['TargetNodeList']
                }
            yield self.controller.timeout(self.refresh_rate)

    def ballot_stuffing_attack(self):
        trusted_candidates = [n for n in self.global_trust.keys() if not isinstance(n, ZAMMalicious) and n.get_online()]
        ...
        malicious_nodes = [n for n in self.global_trust.keys() if isinstance(n, ZAMMalicious) and n.get_online()]
        for malicious_node in malicious_nodes:
            ...
            attack_entry = {"attacking_node": malicious_node.name, "attack_type": "ballot stuffing"}
        ...
        for _, node in self.scenario.get_nodes().items():
            if isinstance(node, ZAMNode):
                if node.get_is_executing() or node.isBusy:
                    continue
                if node.get_online() and random.random() < OFFLINE_PROBABILITY:
                    node.set_online(False)
                elif not node.get_online() and random.random() < ONLINE_PROBABILITY:
                    node.set_online(True)
    
    def accumulate_PR(self, target: ZAMNode) -> float:
        total_trust = sum(trust for node, trust in self.global_trust.items() if node != target)
        ...
        for node in self.scenario.get_nodes().values():
            if node != target and isinstance(node, ZAMNode):
                try:
                    ...
                    peerRating = 0.0
                trust_w = trust_weights.get(node, 0.0)
    ...
    def computeQoS(self):
        ...
        for message in self.trust_messages:
            dst = self.scenario.get_node(message[1])
            if dst is None or (hasattr(dst, "get_online") and not dst.get_online()):
                continue
            oldQos = dst.get_QoS()
            exec_flag = message[3]
            src = self.scenario.get_node(message[0])
